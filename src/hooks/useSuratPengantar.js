// src/hooks/useSuratPengantar.js - FIXED VERSION untuk masalah useMyRequests
...
export const useSuratPengantar = () => {
    ...
    // ===== MY REQUESTS - FIXED: Buat sebagai function yang bisa dipanggil =====
    const useMyRequests = useCallback((params = {}) => {
        const validatedParams = useValidatedParams(params);
        
        // Debug log
        console.log('useMyRequests called with params:', params);
        console.log('Validated params:', validatedParams);
        
        return useQuery({
            queryKey: [QUERY_KEYS.MY_REQUESTS, validatedParams],
            queryFn: async () => {
                console.log('Fetching my requests with params:', validatedParams);
                try {
                    const result = await suratPengantarService.getMyRequests(validatedParams);
            ...
            onSuccess: (data) => {
                console.log('Query success with data:', data);
            }
        });
    }, []);
    ...
    // ===== DETAIL QUERY =====
    const useDetailQuery = useCallback((id) => {
        return useQuery({
    ...
    // ===== STATISTICS QUERIES =====
    const useStatistics = useCallback((params = {}) => {
    ...
    const usePendingRT = useCallback((params = {}) => {
        const validatedParams = useValidatedParams(params);
    ...
    const usePendingRW = useCallback((params = {}) => {
        const validatedParams = useValidatedParams(params);
    ...
    // ===== ADMIN STATISTICS =====
    const useAdminStatistics = useCallback((params = {}) => {
        const validatedParams = useMemo(() => {
            const validated = {};
    ...
    return {
        ...
        // ✅ FIXED: Query hooks for components to use
        useMyRequests, // Pastikan ini ada dan berfungsi
        useDetailQuery,
        useStatistics,
        usePendingRT,
        usePendingRW,
